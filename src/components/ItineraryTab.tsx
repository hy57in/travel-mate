...
import { Trip, ItineraryItem, DialogState, WeatherForecast } from "../types";
...
interface ItineraryTabProps {
  trip: Trip;
  expandedDay: number;
  setExpandedDay: (d: number) => void;
  sortDayItems: (di: number) => void;
  reorderItems: (di: number, oldIdx: number, newIdx: number) => void;
  addDay: () => void;
  deleteDay: (di: number) => void;
  updateDay: (di: number, updates: Record<string, unknown>) => void;
  todayDayIndex: number;
  weather: WeatherForecast[] | null;
  setDialog: (d: DialogState) => void;
}

function SortableItem({ id, di, ii, item, isNext, setDialog }: SortableItemProps) {
  ...
  return (
    <div ref={setNodeRef} style={{ ...style, display: "flex", alignItems: "center", gap: S.sm, padding: `${S.sm}px ${S.md}px`, borderRadius: T.rSm, transition: "all 0.15s", background: isDragging ? "rgba(255,255,255,0.95)" : isNext ? T.mintLight : item.hl ? T.coralLight : item.pend ? T.amberLight : "transparent", borderLeft: isNext ? `3px solid ${T.mint}` : item.hl ? `3px solid ${T.coral}` : item.pend ? `3px solid ${T.amber}` : "3px solid transparent", boxShadow: isDragging ? T.shadowLg : isNext ? `0 0 0 1px ${T.mint}33` : "none" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: S.sm, flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => setDialog({ type: "item", dayIdx: di, itemIdx: ii })}>
        ...
        {(item.lat || item.url) && (
          <a href={item.lat ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.text)}` : item.url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 16, textDecoration: "none", flexShrink: 0, lineHeight: 1 }}>📍</a>
        )}
...
export default function ItineraryTab({ trip, expandedDay, setExpandedDay, sortDayItems, reorderItems, addDay, deleteDay, updateDay, todayDayIndex, weather, setDialog }: ItineraryTabProps) {
  ...
  const handleDragEnd = (di: number) => (event: { active: { id: string | number }; over: { id: string | number } | null }) => {
