...
export function RouteLine({ points, color, animate }) {
  const map = useMap();
  useEffect(() => {
...
export function AutoBounds({ points, searchPos }) {
  const map = useMap();
  useEffect(() => {
...
export function MapMarkers({ visibleDays, searchResult, onSelect, selectedId, selectedDay }) {
