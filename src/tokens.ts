...
export const CAT: Record<string, { color: string; bg: string; emoji: string }> = {
...
export const toY = (k: number, r: number): number => Math.round(k / r);
export const toK = (y: number, r: number): number => Math.round(y * r);
