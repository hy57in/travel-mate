...
import type { Trip, Expense, ChecklistItem } from "../types";
import type { User } from "@supabase/supabase-js";
import type { RealtimeChannel } from "@supabase/supabase-js";

const nextId = (arr: Array<{ id: number }>): number => Math.max(0, ...arr.map((x) => x.id)) + 1;
...
const rowToTrip = (row: SupabaseRow): Trip => ({ id: row.id, ...row.data, name: row.name, emoji: row.emoji } as Trip);
const tripToRow = (trip: Trip, userId: string): SupabaseRow => {
  const { id, name, emoji, ...data } = trip;
  return { id, owner_id: userId, name: name || "", emoji: emoji || "✈️", data };
};

export default function useTrips(user: User | null) {
    ...
    if (supabase && user) {
      supabase!.from("trips").select("*").order("created_at").then(({ data, error }) => {
        if (!error && data?.length) {
          setTrips(data.map((r) => rowToTrip(r as unknown as SupabaseRow)));
      ...
      const channel = supabase!
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "trips" }, (payload) => {
          if (payload.eventType === "UPDATE") {
            const t = rowToTrip(payload.new as unknown as SupabaseRow);
            setTrips(prev => prev.map(p => p.id === t.id ? t : p));
          } else if (payload.eventType === "INSERT") {
            const t = rowToTrip(payload.new as unknown as SupabaseRow);
            setTrips(prev => prev.some(p => p.id === t.id) ? prev : [...prev, t]);
          } else if (payload.eventType === "DELETE") {
            setTrips(prev => prev.filter(p => p.id !== (payload.old as { id: string }).id));
          }
        })
        .subscribe();
  ...
  const persist = useCallback(
    (nextTrips: Trip[], nextActiveId?: string) => {
      setTrips(nextTrips);
