...
async function load() { if (!stOk()) return null; try { const r = await window.storage.get("tp-v6"); return r?.value ? JSON.parse(r.value) : null; } catch { return null; } }
async function save(d) { if (!stOk()) return; try { await window.storage.set("tp-v6", JSON.stringify(d)); } catch {} }
...
export default function App() {
  ...
  useEffect(() => {
    let m = true;
    (async () => {
      try { const s = await Promise.race([load(), new Promise(r => setTimeout(() => r(null), 1500))]); if (!m) return; if (s?.trips?.length) { setTrips(s.trips); setAid(s.aid || s.trips[0]?.id); } } catch {}
      if (m) setLoading(false);
    })();
    return () => { m = false; };
  }, []);

  const persist = useCallback(async (nt, na) => { setTrips(nt); if (na) setAid(na); await save({ trips: nt, aid: na || aid }); }, [aid]);
  const trip = trips?.find(t => t.id === aid);
  const ut = useCallback(u => { persist(trips.map(t => t.id === aid ? { ...t, ...u } : t)); }, [trips, aid, persist]);
  const reset = async () => { setTrips(DEF); setAid(DEF[0].id); await save({ trips: DEF, aid: DEF[0].id }); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: `linear-gradient(180deg, ${T.peach} 0%, ${T.cream} 30%, ${T.sand} 100%)`, fontFamily: "'Outfit', 'Pretendard', -apple-system, sans-serif", paddingBottom: 100, position: "relative" }}>
      ...
      <Dialog open={!!dlg} onOpenChange={() => setDlg(null)}>
        <DialogContent style={{ maxWidth: 380, borderRadius: T.r + 4, padding: S.xxl }}>
          {dlg?.type === "item" && <>
            <DialogHeader><DialogTitle style={{ fontSize: 16, fontWeight: 700 }}>{dlg.isNew ? "📌 일정 추가" : "✏️ 일정 수정"}</DialogTitle></DialogHeader>
          ...
          {dlg?.type === "addExp" && <><DialogHeader><DialogTitle style={{ fontSize: 16, fontWeight: 700 }}>💰 경비 추가</DialogTitle></DialogHeader><AddExpForm rate={trip.rate} onAdd={exp => { ut({ expenses: [...trip.expenses, { ...exp, id: nid(trip.expenses) }] }); setDlg(null); }} /></>}
          {dlg?.type === "addChk" && <><DialogHeader><DialogTitle style={{ fontSize: 16, fontWeight: 700 }}>✅ 체크리스트 추가</DialogTitle></DialogHeader><AddChkForm onAdd={item => { ut({ checklist: [...trip.checklist, { ...item, id: nid(trip.checklist), done: false }] }); setDlg(null); }} /></>}
          {dlg?.type === "settings" && <><DialogHeader><DialogTitle style={{ fontSize: 16, fontWeight: 700 }}>⚙️ 여행 설정</DialogTitle></DialogHeader><SettingsForm trip={trip} onSave={upd => { ut(upd); setDlg(null); }} /></>}
          {dlg?.type === "trip" && <><DialogHeader><DialogTitle style={{ fontSize: 16, fontWeight: 700 }}>🌏 여행 관리</DialogTitle></DialogHeader>
      ...
      <Dialog open={!!confirmDel} onOpenChange={() => setConfirmDel(null)}>
        <DialogContent style={{ maxWidth: 300, borderRadius: T.r + 4, padding: S.xxl, textAlign: "center" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: S.md }}>
      ...
      <div style={{ display: "flex", gap: S.lg, fontSize: 12, color: T.textSoft }}>
        <label style={{ display: "flex", alignItems: "center", gap: S.xs }}><Switch checked={hl} onCheckedChange={setHl} />핵심</label>
        <label style={{ display: "flex", alignItems: "center", gap: S.xs }}><Switch checked={skip} onCheckedChange={setSkip} />스킵</label>
        <label style={{ display: "flex", alignItems: "center", gap: S.xs }}><Switch checked={pend} onCheckedChange={setPend} />미정</label>
      </div>
      <div style={{ display: "flex", gap: S.sm }}>
...
function AddExpForm({ rate, onAdd }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: S.md }}>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: S.sm, fontSize: 12, color: T.textSoft }}><Switch checked={ok} onCheckedChange={setOk} />확정 비용</label>
