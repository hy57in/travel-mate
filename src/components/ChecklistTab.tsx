...
import { Trip, ChecklistItem, DialogState, ConfirmDeleteState } from "../types";

interface SortableCheckItemProps {
  id: string;
  item: ChecklistItem;
  editingCheckId: number | null;
  setEditingCheckId: (id: number | null) => void;
...
interface ChecklistTabProps {
  ...
  setChecklistFilter: (f: string) => void;
...
const CHK_CAT_COLOR: Record<string, string> = {
  "\uC608\uC57D": T.coral,
  "\uC11C\uB958": T.indigo,
  "\uC900\uBE44": T.mint,
  "\uC9D0\uC2F8\uAE30": T.amber,
...
function SortableCheckItem({ id, item, editingCheckId, setEditingCheckId, setConfirmDelete, updateTrip, trip }: SortableCheckItemProps) {
...
export default function ChecklistTab({
  ...
  const handleDragEnd = (event: { active: { id: string | number }; over: { id: string | number } | null }) => {
    ...
    const oldIdx = filteredChecklist.findIndex(c => `chk-${c.id}` === String(active.id));
    const newIdx = filteredChecklist.findIndex(c => `chk-${c.id}` === String(over.id));
